    ...
    fetch('/api/rides')
        ...
        .then(data => {
            ...
            if (data.rides && data.rides.length > 0) {
                console.log(`${data.rides.length} trajets récupérés depuis l'API`);
                displayRides(data.rides);
                updateResultsCount(data.rides.length);
            } else {
                console.log('Aucun trajet trouvé dans l\'API');
        ...
        .catch(error => {
            ...
            showNotification('Impossible de charger les trajets. Les données de démonstration sont affichées.', 'warning');
            // Les cartes statiques restent visibles en cas d'erreur
        })
        .finally(() => {
            // Masquer le spinner
            if (loadingSpinner) {
...
function displayRides(rides) {
    ...
    // Vider la liste existante (cartes statiques)
    ridesList.innerHTML = '';
...
function createRideCard(ride) {
    ...
    div.dataset.price = ride.price_per_seat;
    div.dataset.rating = ride.driver_rating || 4.5;
    div.dataset.ecological = ride.is_ecological || ride.fuel_type === 'electrique';
    div.dataset.duration = calculateDurationInMinutes(ride.departure_city, ride.arrival_city);
    div.dataset.rideId = ride.id;

    const departureTime = new Date(ride.departure_datetime).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' });
    const arrivalTime = new Date(new Date(ride.departure_datetime).getTime() + (calculateDurationInMinutes(ride.departure_city, ride.arrival_city) * 60000)).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' });

    const isEcological = ride.is_ecological || ride.fuel_type === 'electrique';
    const ecoClass = isEcological ? 'eco-badge' : 'badge bg-secondary';
    const ecoText = isEcological ? '<i class="fas fa-leaf"></i> Écologique' :
                   ride.fuel_type === 'hybride' ? '<i class="fas fa-leaf"></i> Hybride' :
                   '<i class="fas fa-gas-pump"></i> Essence';
            ...
            <div class="col-md-8">
                <div class="driver-info">
                    ...
                    <div>
                        ...
                        <div class="rating">
                            ${generateStars(ride.driver_rating || 4.5)}
                            <span class="text-muted ms-1">${ride.driver_rating || 4.5} (${Math.floor(Math.random() * 30) + 5} avis)</span>
                        </div>
                    </div>
                    <span class="${ecoClass} ms-auto">
                        ${ecoText}
                    </span>
                </div>
                ...
                <div class="route-info mb-2">
                    ...
                    <small class="text-muted">
                        <i class="fas fa-clock me-1"></i>${calculateDurationText(ride.departure_city, ride.arrival_city)}
                        <i class="fas fa-car ms-3 me-1"></i>${ride.brand || 'Véhicule'} ${ride.model || ''} - ${ride.color || 'Couleur inconnue'}
                    </small>
                </div>

                <div class="ride-details">
                    <p class="mb-2 small">${ride.description || 'Trajet confortable et sûr.'}</p>
                    <div class="d-flex gap-3 small text-muted">
                        <span><i class="fas fa-users me-1"></i><span class="seats-available">${ride.available_seats}</span> places restantes</span>
                        <span><i class="fas ${ride.pets_allowed ? 'fa-paw' : 'fa-ban'} me-1"></i>${ride.pets_allowed ? 'Animaux OK' : 'Pas d\'animaux'}</span>
                        <span><i class="fas fa-smoking-ban me-1"></i>Non-fumeur</span>
                    </div>
                </div>
            </div>
...
function calculateDurationInMinutes(departure, arrival) {
    const durations = {
        'Paris_Lyon': 270,
        'Lyon_Marseille': 195,
        'Paris_Bordeaux': 345,
        'Toulouse_Montpellier': 150,
        'Lille_Bruxelles': 105
    };

    const key = departure + '_' + arrival;
    const reverseKey = arrival + '_' + departure;

    return durations[key] || durations[reverseKey] || 180;
}

function calculateDurationText(departure, arrival) {
    const minutes = calculateDurationInMinutes(departure, arrival);
...
function viewRideDetails(rideId) {
    ...
    fetch(`/api/rides/details?id=${rideId}`)
        .then(response => response.json())
        .then(data => {
            if (data.error) {
            ...
            if (data.ride) {
                console.log('Détails du trajet récupérés:', data.ride);
                showRideModal(data.ride);
            } else {
